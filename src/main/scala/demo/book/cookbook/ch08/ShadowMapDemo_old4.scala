...
import yage.base.opengl.framebuffer.FrameBuffer.Target.ReadDraw
...
object ShadowMapDemo_old4 extends GlApp:
  ...
  override def init() =
    ...
    if !frameBuffer.isComplete then throw RuntimeException("bad fbo")
  ...
  def renderShadowMap() =
    GL20C.glUseProgram(prog1.id)
    // Set MVP matrix of the "light camera"
    prog1.setUniform("viewProjectionMatrix", light)
    GL30C.glBindFramebuffer(GL30C.GL_FRAMEBUFFER, frameBuffer.id)
  ...
  def renderNormal() =
    ...
    GL11C.glBindTexture(GL11C.GL_TEXTURE_2D, depthTexture.id)
    GL30C.glBindVertexArray(vao)
    GL11C.glDrawArrays(GL11C.GL_TRIANGLES, 0, 6 * 6 * boxes.size)
    GL30C.glBindVertexArray(0)
