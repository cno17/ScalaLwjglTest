...
trait TextureLoader: // extends StackUser
  ...
  def loadTexture2(img: BufferedImage) =
    val sx = img.getWidth
    val sy = img.getHeight
    val pixels = new Array[Int](sx * sy)
    img.getRGB(0, 0, sx, sy, pixels, 0, sx)
    val buffer = BufferUtils.createByteBuffer(sx * sy * 4) // 4 because RGBA
    for y <- 0 to sy - 1 do
      for x <- 0 to sx - 1 do
        val pixel = pixels(x + y * sx)
    ...
    val tex = new Texture2(TexelFormat.FP32_4, 1, sx, sy)
    tex.load(buffer, TexelFormat.UI08_4)
    tex
